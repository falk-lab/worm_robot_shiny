...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_btn, {
    ...
    # Activity Worm Fraction Stats (Tukey HSD)
    ...
    # Activity Z-Score Stats (Tukey HSD)
  ...
  output$plot_main <- renderPlot({
    ...
    # --- Your full plot logic unchanged ---
    # (All original plotting code from your message remains exactly as-is)
    # ...
  })
  ...
  output$table_main <- renderDT({
    ...
    switch(table_sel,
           "activity_table" = {
             ...
             datatable(df, extensions = 'Buttons', options = list(dom='Blfrtip',
           ...
           "activity_wf_stats" = {
             df <- processed_data$activity_wf_stats
             datatable(df, extensions='Buttons', options=list(dom='Blfrtip',
                                                              buttons=c('copy','csv','excel'),
                                                              lengthMenu=list(c(10,25,50,-1), c(10,25,50,"All"))))
           },
           "activity_zscore_stats" = {
             df <- processed_data$activity_zscore_stats
             datatable(df, extensions='Buttons', options=list(dom='Blfrtip',
                                                              buttons=c('copy','csv','excel'),
                                                              lengthMenu=list(c(10,25,50,-1), c(10,25,50,"All"))))
           },
           "day_max_table" = {
             df <- processed_data$day_max_act
             datatable(df, extensions='Buttons', options=list(dom='Blfrtip',
                                                              buttons=c('copy','csv','excel'),
                                                              lengthMenu=list(c(10,25,50,-1), c(10,25,50,"All"))))
           }
