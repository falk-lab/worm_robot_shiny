...
server <- function(input, output, session) {
    ...
    tags$div(
      ...
      tags$div(
        ...
        tags$div(
          ...
          fileInput("data_file2", label = NULL, accept = ".xlsx"),
  ...
  uploaded_data <- reactive({
    ...
    ext <- tools::file_ext(input$data_file2$name)
    validate(need(ext == "xlsx", "Please upload an Excel (.xlsx) file."))
    read_excel(input$data_file2$datapath)
  })
  
  uploaded_data <-read_excel("C:/Users/akulm/OneDrive/Desktop/Akul/CHOP Research/Reading Excel Data Practice/data.xlsx")
  ...
  output$plot_main <- renderPlot({
    switch(current_plot(),
           plot1 = {
             df <- uploaded_data()
             validate(
               need(ncol(df) >= 2, "Uploaded data must have at least two columns.")
             )
             plot(df[[1]], df[[2]],
                  main = "Plot 1: Uploaded Excel Data",
