  ...
  sidebarLayout(
    sidebarPanel(
      ...
      radioButtons("data_source", "Data Source",
                   choices = c("Single CSV (Original)" = "csv",
...
server <- function(input, output, session) {
  ...
  observeEvent(input$run_btn, {
    ...
    if (input$data_source == "zip") {
      ...
      plate_maps <- readxl::read_excel(input$plate_map_file$datapath, sheet = 1, skip = 12) %>%
        ...
        nest() %>%
        mutate(data = map(data, ~ platetools::plate_matrix(data = .$well_info, well = .$well, plate = 24)),
      ...
      for (j in worm_robot_files) {
        temp <- vroom(unz(input$zip_file$datapath, j), delim = ',') %>%
      ...
      for (k in worm_robot_files) { 
        ...
        temp <- vroom(unz(input$zip_file$datapath, k), delim = ',', skip = temp_locations_worm_frac$start_rownum + 1,
                      col_names = paste0(1:24), col_types = 'dddddddddddddddddddddddd') %>%
          head(1) %>%
      ...
      for (l in worm_robot_files) { 
        ...
        temp <- vroom(unz(input$zip_file$datapath, l), delim = ',', skip = temp_locations_roi_activity$start_rownum[1] + 1,
                      col_names = paste0(1:24), col_types = 'dddddddddddddddddddddddd') %>%
          head(n_rows) %>%
          mutate(picture = 1:n(),
                 file_path = l) %>%
      ...
      raw_data <- roi_activity %>%
        ...
        mutate(plate = if_else(is.na(plate), 1, plate),
    ...
    # ----------------------------
    # ORIGINAL ANALYSIS CODE (fully unchanged)
    # ----------------------------
    ...
    switch(plot_sel,
           ...
           "combined" = {
             lifespan %>%
               ...
               pivot_longer(cols = c("days_max","healthspan","lifespan"), names_to = 'measure', values_to = 'days_val') %>%
               ggplot(aes(x = well_info2, y = days_val, color = measure)) +
               geom_boxplot() +
               theme_bw() +
               labs(x = 'Condition', y = 'Days', color = 'Measure')
           }
    )
  })
  ...
  output$table_main <- renderDT({
    ...
    switch(tbl_sel,
           ...
           "activity_table" = processed_data$activity_ranked %>% select(plate, start_date, well, well_info, mean_activity, worm_fraction),
           "lifespan_table" = processed_data$lifespan %>% select(plate, start_date, well, well_info, healthspan, lifespan),
           "day_max_table" = processed_data$day_max_act %>% select(plate, start_date, well, well_info, days),
           "activity_wf_stats" = processed_data$activity_wf_stats,
           "activity_zscore_stats" = processed_data$activity_zscore_stats
    )
  })
